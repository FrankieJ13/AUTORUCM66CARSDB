(function () {
  ...
  // ============ КАРТОЧКА (22:9, контент слева, фото справа) ============
  function card(c) {
    ...
    // плотные инлайн-строки фактов — без сеток-плиток
    const line1 = [c.year, c.mileage ? fmtMileage(c.mileage) : '', [c.engine, c.transmission].filter(Boolean).join(' ')]
      .filter(Boolean).join(' · ');
    const line2 = [c.body, c.drive, c.seats ? c.seats + ' мест' : ''].filter(Boolean).join(' · ');
    const line3 = [
      c.owners ? c.owners + (c.owners == 1 ? ' владелец' : ' владельца') : '',
      c.condition, c.pts, c.trim, c.color
    ].filter(Boolean).join(' · ');
    ...
    el.innerHTML =
      '<div class="card__body">' +
        ...
        '</div>' +
        (line1 ? '<div class="card__row card__row--strong">' + esc(line1) + '</div>' : '') +
        (line2 ? '<div class="card__row">' + esc(line2) + '</div>' : '') +
        (line3 ? '<div class="card__row card__row--muted">' + esc(line3) + '</div>' : '') +
      '</div>' +
